#!/usr/bin/env python3
"""Diagnostica efeito de composição dos compradores elegíveis abril→maio/2025.

Separa compradores comuns e novos elegíveis em maio para explicar a diferença
...
RES = ROOT / "results"
OUT = RES / "composicao_elegiveis_abr_mai_2025"
...
def main() -> None:
    ...
    comp = a_common[["orgao_cnpj"] + METRICS].merge(
        b_common[["orgao_cnpj"] + METRICS], on="orgao_cnpj", suffixes=("_abr", "_mai"), validate="one_to_one"
    )
    for c in METRICS:
        comp[f"delta_{c}"] = comp[f"{c}_mai"] - comp[f"{c}_abr"]
    comp.to_csv(OUT / "mudancas_compradores_comuns.csv", index=False, encoding="utf-8-sig")

    groups = [
        summarize(a, "todos_abr"),
        summarize(b, "todos_mai"),
        summarize(a_common, "comuns_abr"),
        summarize(b_common, "comuns_mai"),
        summarize(b_new, "novos_elegiveis_mai"),
    ]
    ...
    summary = {
        "elegiveis_abr": int(len(a)),
        "elegiveis_mai": int(len(b)),
        "comuns": int(len(common)),
        "novos_elegiveis_maio": int(len(entrants)),
        "saidas_da_elegibilidade": int(len(exits)),
        "crescimento_elegiveis_pct": float((len(b) / len(a) - 1) * 100),
        "hhi_agregado_mediana_abr": med(a, "portfolio_hhi"),
        "hhi_agregado_mediana_mai": med(b, "portfolio_hhi"),
        "hhi_comuns_mediana_abr": med(a_common, "portfolio_hhi"),
        "hhi_comuns_mediana_mai": med(b_common, "portfolio_hhi"),
        "hhi_novos_mediana_mai": med(b_new, "portfolio_hhi"),
        "hhi_norm_novos_mediana_mai": med(b_new, "portfolio_hhi_norm"),
        "n_fornecedores_novos_mediana_mai": med(b_new, "n_fornecedores"),
        "n_instrumentos_novos_mediana_mai": med(b_new, "n_instrumentos"),
        "delta_mediano_dentro_comuns": delta_common,
        "gap_mediana_novos_vs_comuns_maio": entrant_gap,
