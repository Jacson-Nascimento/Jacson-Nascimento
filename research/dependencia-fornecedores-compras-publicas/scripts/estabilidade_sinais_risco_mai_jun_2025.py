#!/usr/bin/env python3
"""Estabilidade longitudinal dos sinais de risco entre jan-mai e jan-jun/2025.
...
RES = ROOT / "results"
OUT = RES / "estabilidade_sinais_risco_mai_jun_2025"
...
def main() -> None:
    ...
    m = aa[keep].merge(bb[keep], on="orgao_cnpj", suffixes=("_mai", "_jun"), validate="one_to_one")

    corr_metrics = [
        "portfolio_hhi", "portfolio_hhi_norm", "portfolio_cr1", "portfolio_cr4",
        "count_hhi", "count_hhi_norm", "exposicao_strength_global", "exposicao_degree_global"
    ]
    correlations = {k: rho(m[f"{k}_mai"], m[f"{k}_jun"]) for k in corr_metrics}

    overlaps = {}
    for k in ["portfolio_hhi", "portfolio_hhi_norm", "exposicao_strength_global", "exposicao_degree_global"]:
        qa = float(m[f"{k}_mai"].quantile(.75)); qb = float(m[f"{k}_jun"].quantile(.75))
        sa = set(m.loc[m[f"{k}_mai"].ge(qa), "orgao_cnpj"].astype(str))
        sb = set(m.loc[m[f"{k}_jun"].ge(qb), "orgao_cnpj"].astype(str))
        overlaps[k] = {
            "n_top_mai": len(sa), "n_top_jun": len(sb), "n_intersecao": len(sa & sb),
            "jaccard": jaccard(sa, sb),
            "retencao_mai_pct": None if not sa else len(sa & sb) / len(sa) * 100,
        }

    hidden_a = set(m.loc[m["exposicao_oculta_mai"], "orgao_cnpj"].astype(str))
    hidden_b = set(m.loc[m["exposicao_oculta_jun"], "orgao_cnpj"].astype(str))
    hidden = {
        "n_mai": len(hidden_a), "n_jun": len(hidden_b), "n_persistentes": len(hidden_a & hidden_b),
        "jaccard": jaccard(hidden_a, hidden_b),
        "retencao_mai_pct": None if not hidden_a else len(hidden_a & hidden_b) / len(hidden_a) * 100,
        "novos_em_jun": len(hidden_b - hidden_a), "saidas_em_jun": len(hidden_a - hidden_b),
    }

    transition = pd.crosstab(m["quadrante_mai"], m["quadrante_jun"], margins=True, dropna=False)
    transition.to_csv(OUT / "matriz_transicao_quadrantes.csv", encoding="utf-8-sig")

    m["delta_hhi"] = m["portfolio_hhi_jun"] - m["portfolio_hhi_mai"]
    m["delta_hhi_norm"] = m["portfolio_hhi_norm_jun"] - m["portfolio_hhi_norm_mai"]
    m["delta_exposicao_strength"] = m["exposicao_strength_global_jun"] - m["exposicao_strength_global_mai"]
    m["quadrante_estavel"] = m["quadrante_mai"].eq(m["quadrante_jun"])
    m.to_csv(OUT / "comparacao_compradores_comuns.csv", index=False, encoding="utf-8-sig")

    summary = {
        "elegiveis_mai": int(len(a)), "elegiveis_jun": int(len(b)), "compradores_comuns": int(len(m)),
        "retencao_elegiveis_mai_pct": float(len(m) / max(len(a), 1) * 100),
        "cortes_common_sample": {"mai": cut_a, "jun": cut_b},
