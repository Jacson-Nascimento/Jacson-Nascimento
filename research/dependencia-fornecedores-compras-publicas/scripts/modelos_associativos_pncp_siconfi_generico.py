...
Mantém a especificação pré-fixada validada nas janelas anteriores:
...
Não há estratégia causal e nenhuma covariável é selecionada após observar o mês.
"""
...
def parse_args():
    ...
    p.add_argument("--month", type=int, required=True, choices=range(2, 13))
    p.add_argument("--compare-month", type=int, default=None, choices=range(1, 13))
    return p.parse_args()
...
def paths(month: int, compare_month: int | None):
    inp = RES / f"siconfi_integracao_acumulada_2025_{month:02d}" / f"painel_compradores_pncp_siconfi_2025_{month:02d}.csv"
    out = RES / f"modelos_associativos_jan_{month:02d}_2025"
    old = None
    if compare_month is not None:
        old = RES / f"modelos_associativos_jan_{compare_month:02d}_2025" / "coeficientes_modelos.csv"
    return inp, out, old


def prep(inp: Path):
    d = pd.read_csv(inp, dtype={"orgao_cnpj":"string","uf":"string","municipio_ibge":"string"}, low_memory=False)
    nums = ["portfolio_hhi","portfolio_hhi_norm","count_hhi","count_hhi_norm","n_fornecedores","n_instrumentos",
            "despesa_empenhada_2025","populacao_siconfi_2025","exposicao_strength","valor_total"]
    for c in nums:
        if c in d.columns:
...
def tidy(model, name, kind):
    ci = model.conf_int()
    return pd.DataFrame({
        "modelo": name, "tipo": kind, "termo": model.params.index,
        "coef": model.params.values, "erro_padrao": model.bse.values,
        "estatistica": model.tvalues.values, "p": model.pvalues.values,
...
def frac(a, name, formula):
    m = smf.glm(formula, data=a, family=sm.families.Binomial()).fit(
        cov_type="cluster", cov_kwds={"groups": a.municipio_ibge, "use_correction": True}
    )
...
def compare_old(new, old_path: Path | None, compare_month: int | None, month: int):
    if old_path is None or not old_path.exists() or compare_month is None:
        return pd.DataFrame()
    old = pd.read_csv(old_path, low_memory=False)
    rows = []
    for model in ["O2_HHI_norm_regiao", "F2_HHI_norm_regiao"]:
        for term in KEY:
            a = old[(old.modelo == model) & (old.termo == term)]
            b = new[(new.modelo == model) & (new.termo == term)]
            if a.empty or b.empty:
                continue
            ro, rn = a.iloc[0], b.iloc[0]
            co, cn = float(ro.coef), float(rn.coef)
            rows.append({
                "modelo": model, "termo": term,
                f"coef_m{compare_month:02d}": co, f"p_m{compare_month:02d}": float(ro.p),
                f"coef_m{month:02d}": cn, f"p_m{month:02d}": float(rn.p),
                "mesmo_sinal": bool(np.sign(co) == np.sign(cn)),
                "delta_coef": cn - co,
                "razao_abs_coef": None if abs(co) < 1e-12 else abs(cn) / abs(co),
                f"significativo_5pct_m{compare_month:02d}": bool(float(ro.p) < .05),
                f"significativo_5pct_m{month:02d}": bool(float(rn.p) < .05),
            })
...
def main():
    args = parse_args()
    month = args.month
    compare_month = args.compare_month if args.compare_month is not None else month - 1
    if compare_month >= month:
        raise ValueError("--compare-month deve ser menor que --month")
    inp, out, old = paths(month, compare_month)
    if not inp.exists():
    ...
    vv = vif(a)
    vv.to_csv(out / "diagnostico_vif.csv", index=False, encoding="utf-8-sig")
    ...
    comp = compare_old(coefs, old, compare_month, month)
    comp.to_csv(out / f"comparacao_coeficientes_m{compare_month:02d}_m{month:02d}.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        "natureza": "Modelos associativos; sem interpretação causal.",
        "mes_final_publicacao": f"2025-{month:02d}",
        "mes_comparacao": f"2025-{compare_month:02d}",
        "especificacao": "Especificação pré-fixada; nenhuma seleção de covariáveis após observar o mês.",
        "n": int(len(a)), "clusters_municipio": int(a.municipio_ibge.nunique()),
        "regioes": counts.to_dict(orient="records"), "vif": vv.to_dict(orient="records"),
        "OLS_regiao": {
            "populacao": get("O2_HHI_norm_regiao", "log_populacao"),
            "despesa_pc": get("O2_HHI_norm_regiao", "log_despesa_pc"),
            "n_fornecedores": get("O2_HHI_norm_regiao", "log_n_fornecedores"),
            "recorrencia": get("O2_HHI_norm_regiao", "log_instr_por_forn"),
        },
        "fractional_regiao": {
            "populacao": get("F2_HHI_norm_regiao", "log_populacao"),
            "despesa_pc": get("F2_HHI_norm_regiao", "log_despesa_pc"),
            "n_fornecedores": get("F2_HHI_norm_regiao", "log_n_fornecedores"),
            "recorrencia": get("F2_HHI_norm_regiao", "log_instr_por_forn"),
        },
        "gap_regiao": {
            "populacao": get("O3_Gap_regiao", "log_populacao"),
            "despesa_pc": get("O3_Gap_regiao", "log_despesa_pc"),
            "n_fornecedores": get("O3_Gap_regiao", "log_n_fornecedores"),
            "recorrencia": get("O3_Gap_regiao", "log_instr_por_forn"),
        },
        "exposicao_strength_regiao": {
            "populacao": get("O4_Exposicao_strength_regiao", "log_populacao"),
            "despesa_pc": get("O4_Exposicao_strength_regiao", "log_despesa_pc"),
            "n_fornecedores": get("O4_Exposicao_strength_regiao", "log_n_fornecedores"),
            "recorrencia": get("O4_Exposicao_strength_regiao", "log_instr_por_forn"),
        },
        "comparacao_mes_anterior": comp.to_dict(orient="records"),
        "regra_interpretacao": "Priorizar persistência de sinal e ordem de grandeza; significância isolada não é evidência causal."
    }
    (out / "resumo_modelos_generico.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
