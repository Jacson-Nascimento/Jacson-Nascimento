#!/usr/bin/env python3
"""Modelos associativos PNCP × SICONFI para a coorte acumulada jan-abr/2025.

Replica sem alteração a especificação pré-fixada em jan-mar:
...
Compara termo a termo jan-mar versus jan-abr. Não há estratégia causal.
"""
...
RES=ROOT/"results"
INP=RES/"siconfi_integracao_acumulada_2025_04"/"painel_compradores_pncp_siconfi_2025_04.csv"
OLD=RES/"modelos_associativos_jan_mar_2025"/"coeficientes_modelos.csv"
OUT=RES/"modelos_associativos_jan_abr_2025"
...
def compare_old(new):
    ...
    for model in ["O2_HHI_norm_regiao","F2_HHI_norm_regiao"]:
        for term in KEY:
            ...
            ro=a.iloc[0]; rn=b.iloc[0]; co=float(ro.coef); cn=float(rn.coef)
            rows.append({"modelo":model,"termo":term,
                "coef_jan_mar":co,"p_jan_mar":float(ro.p),"coef_jan_abr":cn,"p_jan_abr":float(rn.p),
                "mesmo_sinal":bool(np.sign(co)==np.sign(cn)),"delta_coef":cn-co,
                "razao_abs_coef":None if abs(co)<1e-12 else abs(cn)/abs(co),
                "significativo_5pct_jan_mar":bool(float(ro.p)<.05),"significativo_5pct_jan_abr":bool(float(rn.p)<.05)})
...
def main():
    ...
    _,t1=ols(a,"O1_HHI_norm_externo",f_ext); _,t2=ols(a,"O2_HHI_norm_regiao",f_main)
    _,t3=ols(a,"O3_Gap_regiao",f_gap); _,t4=ols(a,"O4_Exposicao_strength_regiao",f_exp)
    _,tf1=frac(a,"F1_HHI_norm_externo",f_ext); _,tf2=frac(a,"F2_HHI_norm_regiao",f_main)
    ...
    comp=compare_old(coefs); comp.to_csv(OUT/"comparacao_coeficientes_jan_mar_jan_abr.csv",index=False,encoding="utf-8-sig")
    ...
    summary={"natureza":"Modelos associativos; sem interpretação causal.","mes_final_publicacao":"2025-04",
        "especificacao":"Idêntica à pré-fixada em jan-mar; nenhuma seleção de covariáveis após observar abril.",
        ...
        "comparacao_jan_mar":comp.to_dict(orient="records"),
        "regra_interpretacao":"Priorizar persistência de sinal e ordem de grandeza entre jan-mar e jan-abr; significância isolada não é evidência causal."}
