#!/usr/bin/env python3
"""Modelos associativos PNCP × SICONFI para a coorte acumulada jan-jun/2025.

Replica sem alteração a especificação pré-fixada:
...
Compara termo a termo jan-mai versus jan-jun. Não há estratégia causal.
"""
...
RES=ROOT/"results"
INP=RES/"siconfi_integracao_acumulada_2025_06"/"painel_compradores_pncp_siconfi_2025_06.csv"
OLD=RES/"modelos_associativos_jan_mai_2025"/"coeficientes_modelos.csv"
OUT=RES/"modelos_associativos_jan_jun_2025"
    ...
    for model in ["O2_HHI_norm_regiao","F2_HHI_norm_regiao"]:
        for term in KEY:
            ...
            rows.append({"modelo":model,"termo":term,
                "coef_jan_mai":co,"p_jan_mai":float(ro.p),"coef_jan_jun":cn,"p_jan_jun":float(rn.p),
                "mesmo_sinal":bool(np.sign(co)==np.sign(cn)),"delta_coef":cn-co,
                "razao_abs_coef":None if abs(co)<1e-12 else abs(cn)/abs(co),
                "significativo_5pct_jan_mai":bool(float(ro.p)<.05),"significativo_5pct_jan_jun":bool(float(rn.p)<.05)})
...
def main():
    ...
    comp=compare_old(coefs); comp.to_csv(OUT/"comparacao_coeficientes_jan_mai_jan_jun.csv",index=False,encoding="utf-8-sig")
    ...
    summary={"natureza":"Modelos associativos; sem interpretação causal.","mes_final_publicacao":"2025-06",
        "especificacao":"Idêntica à pré-fixada; nenhuma seleção de covariáveis após observar junho.",
        ...
        "comparacao_jan_mai":comp.to_dict(orient="records"),
        "regra_interpretacao":"Priorizar persistência de sinal e ordem de grandeza entre jan-mai e jan-jun; significância isolada não é evidência causal."}
