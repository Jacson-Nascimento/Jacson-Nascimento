#!/usr/bin/env python3
"""Modelos associativos PNCP × SICONFI para a coorte acumulada jan-mai/2025.
...
Compara termo a termo jan-abr versus jan-mai. Não há estratégia causal.
"""
...
RES=ROOT/"results"
INP=RES/"siconfi_integracao_acumulada_2025_05"/"painel_compradores_pncp_siconfi_2025_05.csv"
OLD=RES/"modelos_associativos_jan_abr_2025"/"coeficientes_modelos.csv"
OUT=RES/"modelos_associativos_jan_mai_2025"
    ...
    for model in ["O2_HHI_norm_regiao","F2_HHI_norm_regiao"]:
        for term in KEY:
            ...
            rows.append({"modelo":model,"termo":term,
                "coef_jan_abr":co,"p_jan_abr":float(ro.p),"coef_jan_mai":cn,"p_jan_mai":float(rn.p),
                "mesmo_sinal":bool(np.sign(co)==np.sign(cn)),"delta_coef":cn-co,
                "razao_abs_coef":None if abs(co)<1e-12 else abs(cn)/abs(co),
                "significativo_5pct_jan_abr":bool(float(ro.p)<.05),"significativo_5pct_jan_mai":bool(float(rn.p)<.05)})
...
def main():
    ...
    comp=compare_old(coefs); comp.to_csv(OUT/"comparacao_coeficientes_jan_abr_jan_mai.csv",index=False,encoding="utf-8-sig")
    ...
    summary={"natureza":"Modelos associativos; sem interpretação causal.","mes_final_publicacao":"2025-05",
        "especificacao":"Idêntica à pré-fixada em jan-mar; nenhuma seleção de covariáveis após observar maio.",
        ...
        "comparacao_jan_abr":comp.to_dict(orient="records"),
        "regra_interpretacao":"Priorizar persistência de sinal e ordem de grandeza entre jan-abr e jan-mai; significância isolada não é evidência causal."}
