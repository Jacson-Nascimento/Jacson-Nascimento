...
def main():
    ...
    _,ri,bi,si=metrics(common,"valorInicial")
    _,rg,bg,sg=metrics(common,"valorGlobal")

    e0=set(b0[(b0.n_fornecedores>=3)&(b0.n_instrumentos>=5)].orgao_cnpj.astype(str))
    en=set(bn[(bn.n_fornecedores>=3)&(bn.n_instrumentos>=5)].orgao_cnpj.astype(str))
    common_lag=sorted(e0&en)
    ei=set(bi[(bi.n_fornecedores>=3)&(bi.n_instrumentos>=5)].orgao_cnpj.astype(str))
    eg=set(bg[(bg.n_fornecedores>=3)&(bg.n_instrumentos>=5)].orgao_cnpj.astype(str))
    common_value=sorted(ei&eg)

    desc=pd.DataFrame([
        describe(b0,common_lag,"lag_baseline"),describe(bn,common_lag,"lag_exclui_negativos"),
        describe(bi,common_value,"valorInicial_amostra_comum"),describe(bg,common_value,"valorGlobal_amostra_comum"),
    ])
    desc.to_csv(OUT/"comparacao_metricas.csv",index=False,encoding="utf-8-sig")
